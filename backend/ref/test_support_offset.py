...
# Test function
def evaluate_offset(left_offset, th, min_px):
    # Temporarily monkeypatch splitImg to use custom offset
    def splitImg_custom(img):
        ...
        for y in range(cols):
            for x in range(rows):
                ...
                # Adjust end limit proportionally
    ...
    boxes = splitImg_custom(imgThresh)
...
for offset in range(20, 42, 2):
    for th in [135, 140, 145, 150, 155]:
        for mp in [15, 20, 25, 30]:
            blanks, correct = evaluate_offset(offset, th, mp)
            if correct >= 11:
